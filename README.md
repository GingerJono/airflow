...
| cytora            | `generic` (Generic)             | Login: From key vault secret `cytora-client-id`</br>Password: From key vault secret `cytora-client-secret`</br>Extra: `{"region":"eu", "workspace":"dale_uw_eu_poc"}`                                                                                                    | Connection to Cytora, used for further processing of email contents on cytora.         |
